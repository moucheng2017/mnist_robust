...
class UNetPoE(nn.Module):
    ...
    def forward(self, x):
        ...
        conv1b = F.sigmoid(self.dconv_down1b(x))
        conv1 = conv1a*conv1b
        x = self.maxpool(conv1)

        conv2a = self.dconv_down2a(x)
        conv2b = F.sigmoid(self.dconv_down2a(x))
        conv2 = conv2a * conv2b
        x = self.maxpool(conv2)

        conv3a = self.dconv_down3a(x)
        conv3b = F.sigmoid(self.dconv_down3b(x))
        conv3 = conv3a*conv3b
        x = self.upsample(conv3)

        x = torch.cat([x, conv2], dim=1)
        xa = self.dconv_up2a(x)
        xb = F.sigmoid(self.dconv_up2b(x))
        x = xa*xb

        x = self.upsample(x)
        x = torch.cat([x, conv1], dim=1)
        xa = self.dconv_up1a(x)
        xb = F.sigmoid(self.dconv_up1b(x))
        x = xa*xb
