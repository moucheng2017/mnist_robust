...
class Generalised_GMoE(nn.Module):
    def __init__(self, width, ks, mu=0, sigma=1, dilation=5, num_elayers=None, standard_gate=True, random_gate=None,
                 avg_gate=None, device='cuda'):
        super().__init__()
    ...
    def eval_gate(self, x, moe_layer, gate, noise, num_experts, k):
        ...
        for i in range(num_experts):
            for j in range(batch_size):
                init_output[i, j] = torch.mul(init_output[i, j],
                                              torch.full((init_output[i, j].shape), float(gate[j, i])).to(self.device))
    ...
    def forward(self, x):
        ...
        # mix the experts
        conv1 = self.eval_gate(x, self.dconv_down1, self.gate1, self.noise1, num_experts=self.num_elayers[0],
                               k=self.ks[0])
        x = self.maxpool(conv1)

        # mix the experts
        conv2 = self.eval_gate(x, self.dconv_down2, self.gate2, self.noise2, num_experts=self.num_elayers[1],
                               k=self.ks[1])
        x = self.maxpool(conv2)

        # you guessed it we're mixing the experts again
        conv3 = self.eval_gate(x, self.dconv_down3, self.gate3, self.noise3, num_experts=self.num_elayers[2],
                               k=self.ks[2])
